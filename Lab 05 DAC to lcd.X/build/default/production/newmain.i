...
void main(void) {
    ...
    while(1) {
        ...
        if (key != 0 && is_valid_key(key)) {
            state.voltage_limit = key - '0';
        }


        state.current_voltage = read_adc_voltage();
...
void float_to_display(float value, char *buffer, unsigned char precision) {
    ...
    if (precision == 1) {
        decPart = (unsigned int)((value - intPart) * 10);
    } else if (precision == 2) {
        decPart = (unsigned int)((value - intPart) * 100);
    }
    ...
    if (precision > 0) {
        ...
        if (precision == 2 && decPart < 10) {
            buffer[i++] = '0';
        }


        if (decPart == 0) {
            for (unsigned char j = 0; j < precision; j++) {
                buffer[i++] = '0';
            }
        } else {
            unsigned char digits = 0;
            unsigned int temp = decPart;
...
void update_display(SystemState *state) {
    ...
    putsXLCD("Vlim:");
    WriteCmdXLCD(0x85);
    float_to_display(state->voltage_limit, buffer, 1);
    putsXLCD(buffer);
    ...
    putsXLCD("Vcon:");
    WriteCmdXLCD(0x8C);
    float_to_display(state->current_voltage, buffer, 2);
    putsXLCD(buffer);


    WriteCmdXLCD(0xC0);
    putsXLCD(state->led_state ? "LED: On " : "LED: Off");
}

void update_led(SystemState *state) {
    state->led_state = (state->current_voltage >= state->voltage_limit);
    PORTDbits.RD0 = state->led_state;
}

int is_valid_key(char key) {
    const char *valid = "012345";
    while (*valid) {
        if (key == *valid++) {
            return 1;
        }
